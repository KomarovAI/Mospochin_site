...
Для вентиляционного кластера дополнительно открыть:

```bash
cat docs/VENTILATION_CLUSTER_AI_GUIDE.md
cat data/ventilation-cluster-pages.json
cat data/ventilation-photo-map.json
```
...
Для новой страницы используйте единый lifecycle:

```bash
npm run page:create -- --branch restaurant --slug <slug> --device-name "..." --ui-label "..." --service-name "..." --family <family> --symptoms "..." --brands "..."
npm run page:edit -- --page <file.html>
npm run page:check -- --page <file.html>
```

`page:create` связывает scaffold, metrics context, source bootstrap, builder, metadata, sitemap и проверки. `page:edit` только подготавливает AI-контекст; фактическая правка выполняется в source-слое.

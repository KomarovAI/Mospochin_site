# Page Digest — parokonvektomat-e02-e07-e10.html

- Branch: neutral
- Role: neutral
- Title: Ошибки E02, E07, E10 пароконвектомата — MosPochin
- Description: Ошибки E02, E07, E10 на пароконвектоматах Abat, Абат и Fagor: диагностика на объекте, фото кода в WhatsApp, выезд инженера для ресторанов и кафе.
- H1: Ошибка E02 / E07 / E10
- Canonical: https://mospochin.ru/parokonvektomat-e02-e07-e10.html
- Builder model: src/pages/parokonvektomat-e02-e07-e10/page.json
- Sections: 76 (49 local, 20 shared refs, 37 raw)
- Text words inside referenced sections: 2523

## Component mix

| Component | Count |
| --- | --- |
| raw | 37 |
| pricing | 10 |
| proof | 9 |
| mobile-contact | 4 |
| related-links | 3 |
| breadcrumb | 2 |
| contact-cta | 2 |
| faq | 2 |
| section | 2 |
| footer-anchor | 1 |
| hero | 1 |
| lead-form | 1 |
| noscript | 1 |
| runtime-partials | 1 |


## Largest sections to inspect first

| Section | Component | Bytes | Words | Shared | Source |
| --- | --- | --- | --- | --- | --- |
| Что означают ошибки E02, E07, E10 | mobile-contact | 13.8 KB | 251 | no | src/pages/parokonvektomat-e02-e07-e10/sections/013-mobile-contact-chto-oznachayut-oshibki-e02-e07-e10.html |
| Ошибка E02 / E07 / E10 | hero | 10.9 KB | 101 | no | src/pages/parokonvektomat-e02-e07-e10/sections/007-hero-oshibka-e02-e07-e10.html |
| Частые поломки пароконвектоматов | pricing | 10.6 KB | 197 | yes | src/components/shared/pricing/pricing-chastye-polomki-parokonvektomatov--49df61a84592b2cf.html |
| Последние ремонты пароконвектоматов | pricing | 8.9 KB | 124 | yes | src/components/shared/pricing/pricing-poslednie-remonty-parokonvektomatov--e1cca13f78f777a0.html |
| Ремонтируем все типы пароконвектоматов | pricing | 7.4 KB | 134 | yes | src/components/shared/pricing/pricing-remontiruem-vse-tipy-parokonvektomatov--716e30e482bc8a8d.html |
| Получите понятный сценарий ремонта пароконвектомата | lead-form | 7.2 KB | 104 | no | src/components/parametric/lead-form/restaurant-parokonvektomat-b2b.template.html |
| Частые вопросы о ремонте пароконвектоматов | faq | 6.7 KB | 242 | yes | src/components/shared/faq/faq-chastye-voprosy-o-remonte-parokonvektomatov--8a089715e4395424.html |
| Что фиксируем до начала работ на объекте | pricing | 6.5 KB | 192 | no | src/pages/parokonvektomat-e02-e07-e10/sections/061-pricing-chto-fiksiruem-do-nachala-rabot-na-ob-ekte.html |


## Editable source files

- data/page-metadata.json
- data/contact-config.json
- data/runtime-config.json
- parokonvektomat-e02-e07-e10.html
- src/site-builder.json
- src/pages/parokonvektomat-e02-e07-e10/page.json
- src/pages/parokonvektomat-e02-e07-e10/sections/

## Checks

- npm run doctor:page -- --page parokonvektomat-e02-e07-e10.html
- npm run validate:site
- npm run check:site-builder
- npm run ai:semantic-diff -- --page parokonvektomat-e02-e07-e10.html
- npm run ai:check

## AI notes

- Для правки уникального блока открывай конкретный `sections/*.html`.
- Для shared-блока сначала запускай `npm run ai:impact -- --files <componentRef>`, потому что он может затронуть несколько страниц.
- Если много raw-секций, не переписывай их вслепую: сначала пойми границы блока по соседним секциям.

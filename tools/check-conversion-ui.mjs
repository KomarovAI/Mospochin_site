...
const MANIFEST_PATH = path.join(SITE_ROOT, 'data/parokonvektomat-conversion-pages.json');
...
function validateManifest(manifest, metadata) {
  const errors = [];
  if (!manifest || typeof manifest !== 'object') errors.push('conversion manifest must be an object');
  if (!Array.isArray(manifest.pages) || !manifest.pages.length) errors.push('conversion manifest pages must be a non-empty array');
  if (!manifest.canonicalBase || typeof manifest.canonicalBase !== 'string') errors.push('conversion manifest canonicalBase is required');

  const seen = new Set();
  for (const [index, entry] of (manifest.pages ?? []).entries()) {
    const context = `data/parokonvektomat-conversion-pages.json pages[${index}]`;
    ...
    if (metadataEntry && entry.branch && metadataEntry.branch !== entry.branch) {
      errors.push(`${entry.page}: branch mismatch (${entry.branch} in conversion manifest, ${metadataEntry.branch} in metadata)`);
    }
  }
  return errors;
}
...
function validatePage(entry, manifest, metadata, sitemap, clusterPages) {
  ...
  const minForms = entry.minForms ?? defaults.minForms ?? 1;
  if (formCount < minForms) errors.push(`${entry.page}: expected at least ${minForms} form(s), found ${formCount}`);
  if (!/<form\b[^>]*class=["'][^"']*telegram-form/i.test(html)) errors.push(`${entry.page}: missing .telegram-form`);
  if (defaults.requireHiddenProblem && !/<input\b[^>]*type=["']hidden["'][^>]*name=["']problem["']/i.test(html)) {
    errors.push(`${entry.page}: missing hidden problem input`);
  }
  if (!/<input\b[^>]*name=["']phone["'][^>]*required/i.test(html) && !/<input\b[^>]*required[^>]*name=["']phone["']/i.test(html)) {
    errors.push(`${entry.page}: missing required phone input`);
  }
  if (!/<button\b[^>]*type=["']submit["']/i.test(html)) errors.push(`${entry.page}: missing submit button`);
  ...
  const localHtmlLinks = hrefs.map(normalizeLocalHtmlHref).filter(Boolean);
  ...
  if (clusterLinks.length < minClusterLinks) {
    errors.push(`${entry.page}: expected at least ${minClusterLinks} cluster links, found ${clusterLinks.length}`);
  }

  const metadataEntry = metadata.pages?.[entry.page];
  if (metadataEntry?.canonical && metadataEntry.canonical !== expectedCanonical) {
    errors.push(`${entry.page}: metadata canonical mismatch (${metadataEntry.canonical} != ${expectedCanonical})`);
  }
...
try {
  const manifest = readJson('data/parokonvektomat-conversion-pages.json');
  ...
  const manifestErrors = validateManifest(manifest, metadata);
  const clusterPages = new Set((manifest.pages ?? []).map((entry) => entry.page));
  const pageResults = (manifest.pages ?? []).map((entry) => validatePage(entry, manifest, metadata, sitemap, clusterPages));
  const errors = [...manifestErrors, ...pageResults.flatMap((result) => result.errors)];
  ...
  console.log(`✅ Conversion UI check passed: ${pageResults.length} parokonvektomat pages`);
  pageResults.forEach((result) => {
    console.log(
      `- ${result.page}: forms=${result.forms}, clusterLinks=${result.clusterLinks}, ${result.indexable ? 'indexable' : 'noindex'}`
    );
  });
  if (warnings.length) {
    console.log('\nWarnings:');

...
const manifest=JSON.parse(fs.readFileSync(path.join(root,'data/dishwasher-cluster-pages.json'),'utf8'));
...
const brandModels=JSON.parse(fs.readFileSync(path.join(root,'data/dishwasher-brand-models.json'),'utf8'));
...
for(const p of pages){
 ...
 context.pages[p.page]={...(context.pages[p.page]||{}),page_slug:p.page.replace(/\.html$/,''),page_intent:p.metrics.pageIntent,equipment:p.metrics.equipment,brand:p.pageType==='brand_service'?(brandById.get(p.intent)||p.intent):'',service:p.metrics.service,commercial_segment:p.metrics.commercialSegment,branch:'restaurant',page_version:v,source:'dishwasher_manifest'};
}
fs.writeFileSync(contextPath,JSON.stringify(context,null,2)+'\n');
console.log(`Dishwasher metrics markup applied: ${pages.length} pages`);

...
const out=path.join(root,'.artifacts/screenshots/refrigeration/rf8-review-fast');
fs.mkdirSync(out,{recursive:true});
const manifest=JSON.parse(fs.readFileSync(path.join(root,'data/refrigeration-rf8-screenshot-audit.json'),'utf8'));
const pages=manifest.pages.map(item=>item.page).filter(file => !process.env.RF8_PAGE || file === process.env.RF8_PAGE);
...
for(const file of pages){
 ...
 for(const [id,w,h,mobile] of [['desktop',1440,1080,false],['mobile',393,852,true]].filter(([id]) => !process.env.RF8_DEVICE || id === process.env.RF8_DEVICE)){

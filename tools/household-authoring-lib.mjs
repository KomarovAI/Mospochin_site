...
export const REGISTRY_PATH = path.join(SITE_ROOT, 'data/household-services.json');
export const SLOTS_PATH = path.join(SITE_ROOT, 'data/household-page-slots.json');
export const PROOF_LAYER_PATH = path.join(SITE_ROOT, 'data/household-proof-layer.json');
...
export function loadHouseholdState() {
  return {
...
export function getPublicServiceContext(args) {
  const { metadata, registry, slots, proofLayer } = loadHouseholdState();
  ...
  if (pageMeta.branch !== 'household') {
    throw new Error(`${page} is not a household page`);
  }

  if (!registryEntry) {
    throw new Error(`${page} is not a household service page`);
  }

  if (registryEntry.isShadow) {
    throw new Error(`${page} is a shadow household page; helper command supports only public service pages`);
  }

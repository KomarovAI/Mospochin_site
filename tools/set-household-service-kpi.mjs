...
function printHelp() {
  console.log(`Usage:
  npm run household:set-service-kpi -- --defaults --json '<json>'
  npm run household:set-service-kpi -- --page <file.html> --json '<json>'
  npm run household:set-service-kpi -- --slug <slug> --file <path>

Options:
  --defaults            Update shared serviceKpiDefaults
  --page / --slug       Public household service page for per-page override
...
try {
  ...
  if (isDefaults) {
    const { slots } = loadHouseholdState();
    ...
    if (!args['dry-run']) {
      console.log('Updated shared household service KPI defaults');
    }
    process.exit(0);
  }

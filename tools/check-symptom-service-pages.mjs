...
}if((html.match(/<h1\b/gi)||[]).length!==1)errors.push(`${p.page}: exactly one H1 required`);if(!/data-page-intent="symptom_service"/.test(html))errors.push(`${p.page}: data-page-intent missing`);if(!/"@type"\s*:\s*"Service"/.test(html))errors.push(`${p.page}: Service schema missing`);if(!/"@type"\s*:\s*"WebPage"/.test(html))errors.push(`${p.page}: WebPage schema missing`);if(/"@type"\s*:\s*"FAQPage"/.test(html))errors.push(`${p.page}: retired FAQPage schema must be absent`);if((html.match(/<details\b/gi)||[]).length<2)errors.push(`${p.page}: at least two visible FAQ items required`);for(const f of ['equipment_model','error_code','problem','phone'])if(!new RegExp(`name=["']${f}["']`).test(html))errors.push(`${p.page}: field ${f} missing`);if((html.match(/data-cta-group="internal_link"/g)||[]).length<5)errors.push(`${p.page}: <5 tracked internal links`);for(const id of t.sourceEvidenceIds||[]){if(!ev.has(id))errors.push(`${p.page}: unknown evidence ${id}`);if(!html.includes(`data-evidence-id="${id}"`))errors.push(`${p.page}: evidence ${id} not visible`)}}

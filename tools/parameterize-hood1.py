...
root=Path('.')
cluster=json.load(open(root/'data/hood-cluster-pages.json'))['pages']
pages=[(x.get('page') or x.get('url') or x.get('filename')) for x in cluster]
base=root/'src/components/parametric/hood1'
...
for page,slug,pdir,m,contents in records:
    ...
    for key,idx,props,original,tname in changes:
        ...
        props_rel=f'src/pages/{slug}/props/hood1-{key}.json'
        (props_dir/f'hood1-{key}.json').write_text(json.dumps(props,ensure_ascii=False,indent=2)+'\n')
        for field in ['file','componentRef','props']:
            sec.pop(field,None)
        sec['componentMode']='parametric'; sec['templateRef']=f'src/components/parametric/hood1/{tname}'; sec['propsRef']=props_rel
...
print(f'parameterized hood HOOD1 {len(pages)*5} section refs')

...
root=Path('.')
spec=json.load(open(root/'data/washing-machine-wm2-page-specs.json'))
pages=[x['page'] for x in spec['pages']]
base=root/'src/components/parametric/washing-wm2'; base.mkdir(parents=True,exist_ok=True)
...
# static shell variants for header/footer
shell={}
...
for page,slug,pdir,m,cs in records:
    ...
    for key,idx,props,orig,tname in changes:
        sec=m['sections'][idx]; old=sec.get('file'); rel=f'src/pages/{slug}/props/wm2-{key}.json'; (propsdir/f'wm2-{key}.json').write_text(json.dumps(props,ensure_ascii=False,indent=2)+'\n')
        for fld in ['file','componentRef','props']: sec.pop(fld,None)
        sec['componentMode']='parametric'; sec['templateRef']=f'src/components/parametric/washing-wm2/{tname}'; sec['propsRef']=rel; sec['bytes']=len(orig.encode()); sec['hash']=h16(orig)
        if old and (pdir/old).exists(): (pdir/old).unlink()
    (pdir/'page.json').write_text(json.dumps(m,ensure_ascii=False,indent=2)+'\n')
print('parameterized',len(pages)*4,'WM2 section refs')

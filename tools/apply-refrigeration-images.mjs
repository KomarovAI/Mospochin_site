...
const TEMPLATE = fs.readFileSync(path.join(ROOT,'src/components/parametric/refrigeration-media/section.template.html'),'utf8');
...
function srcset(asset, format) { return asset.outputs.filter((o)=>o.format===format).sort((a,b)=>a.width-b.width).map((o)=>`${o.path} ${o.width}w`).join(', '); }
function card(asset, layout) {
  ...
  const brands = (asset.brandEvidence || []).join(',');
  return `      <figure class="refrigeration-media-card" data-refrigeration-image-id="${asset.id}" data-image-processing="${asset.processing || 'processed-photo'}" data-orientation="${asset.orientation}"${brands ? ` data-brand-evidence="${esc(brands)}"` : ''}>\n`+
    `        <picture>\n`+
...
function stripExisting(html) { return html.replace(/\s*<section class="refrigeration-media-section"[\s\S]*?<\/section>\s*/g,''); }
...
function updateHead(slug, asset) {
  ...
  if (anchor) html = html.replace(anchor,`${tags}${anchor}`); else html = html.replace('</head>',`${tags}</head>`);
...
for (const [slug,usage] of Object.entries(REGISTRY.pageUsage || {})) {
  ...
  updateHead(slug,assetById.get(usage.ogImage || usage.images[0]));
  count++;
}
console.log(`✅ Applied refrigeration media to ${count} source pages`);

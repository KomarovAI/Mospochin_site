...
- `llm_internal_link_funnel_YYYY-MM-DD.csv` — срез «related category link → следующий page → контакт/форма» в рамках clean-сессии.
...
- internal-link funnel smoke: sample related-category click корректно связывается со следующим page view и `phone_click` в той же clean-сессии;
- handoff: PASS;
...
5. Для оценки перекрёстных CTA смотреть `llm_internal_link_funnel_YYYY-MM-DD.csv`: сначала `click_to_contact_rate`, затем объём `click_sessions` и `confidence`.
